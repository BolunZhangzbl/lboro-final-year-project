# -*- coding: utf-8 -*-
"""
Created on Tue Mar 30 11:22:32 2021
...
class TwoUserEncoder1(object):
    ...
    def Draw_Constellation(self, N=1500):
        ...
        if self.n_channel == 2:
            ...
            plt.title('TwoUser Constellation Diagram,(%d,%d)'%(self.n_channel, self.k), fontsize=18)
    ...
    def CalBLER1(self, bertest_data_size, EbNodB_low=0, EbNodB_high=14, EbNodB_num=28):
...
"""
The following code shows how to apply class TwoUserEncoder1
"""        
model_test1 = TwoUserEncoder(M=256, n_channel=2, k=8, emb_k=256, u1_EbNodB_train=7, u2_EbNodB_train=7,
                            train_datasize=10000, alpha=0.5, beta=0.5)
model_test2 = TwoUserEncoder1(M=256, n_channel=2, k=8, emb_k=256, u1_EbNodB_train=7, u2_EbNodB_train=7,
                            train_datasize=10000, alpha=0.5, beta=0.5)
model_test1.Initialize()
model_test2.Initialize()
print('Initialization finished')
'''model_test1.CalBLER(bertest_data_size=50000, EbNodB_low=0, EbNodB_high=14, EbNodB_num=28)
model_test2.CalBLER1(bertest_data_size=50000, EbNodB_low=0, EbNodB_high=14, EbNodB_num=28)
EbNodB_range = list(np.linspace(0, 14, 28))
plt.plot(EbNodB_range, model_test1.ber, 'bo', label='(4,4), average poewr constraint')
plt.plot(EbNodB_range, model_test2.ber, 'k-', label='(4,4), fixed power constraint')
plt.yscale('log')
plt.xlabel('SNR Range: ', fontsize=18)
plt.ylabel('Block Error Rate: ', fontsize=18)
plt.title('TwoUsers BLER (2,2)', fontsize=20)
...
plt.show()'''
model_test1.Draw_Constellation()
model_test2.Draw_Constellation()        
